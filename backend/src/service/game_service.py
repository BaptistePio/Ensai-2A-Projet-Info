import os
from fastapi import HTTPException

from dao.player_dao import PlayerDao
from utils.log_utils import log
from business_object.game_mode.game_mode_factory import GameModeFactory
from business_object.scoring_strategy import ScoringStrategy


class GameService:
    """Service that manages games."""

    @log
    def play(self, id_player: int, id_opponent: int, game_mode: str, **kwargs):
        """Executes a single round of a game between two players.
        Args:
            id_player (int): The unique identifier of the first player.
            id_opponent (int): The unique identifier of the opponent.
            game_mode (str): The identifier of the game mode.
            **kwargs: Additional parameters (e.g., 'choice' for coinflip).
        Returns:
            Game: The resulting Game object.
        Raises:
            HTTPException: 400 if the two players are the same.
            HTTPException: 404 if one or both players are not found.
            ValueError: If the game mode is not supported.
        """
        # 1. Validation des joueurs
        if id_player == id_opponent:
            raise HTTPException(status_code=400, detail="Two different players required")

        p1 = PlayerDao().find_by_id(id_player)
        p2 = PlayerDao().find_by_id(id_opponent)

        if not p1 or not p2:
            raise HTTPException(status_code=404, detail="Player not found")

        # 2. Obtenir le mode de jeu via la Factory
        try:
            mode_engine = GameModeFactory.get_mode(game_mode)
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))

        # 3. Jouer le jeu (on passe les kwargs au mode de jeu)
        # Le mode de jeu s'occupe de la logique et renvoie un objet Game
        game = mode_engine.play(p1, p2, **kwargs)

        # 4. Mise à jour du classement Elo des deux joueurs
        ScoringStrategy.update_player_ratings(game)

        # 5. Sauvegarde des joueurs mis à jour en base de données
        PlayerDao().update(p1)
        PlayerDao().update(p2)

        # Note: Tu pourrais aussi sauvegarder le jeu ici si tu as un GameDao
        # GameDao().save(game)

        # 6. Renvoyer l'objet Game
        return game
